...
#  print STDERR "$plink_mapfile_str\n";
 ...
 #   my $plnk_gt = "0\t0\t"; # this is what plink wants
	...
	($dosage == 2)? "$allele_one\t$allele_one\t" : "0\t0\t";
      push @genotypes_this_row, $plnk_gt;
      $accession_gt_strings[$acc_idx] .= $plnk_gt;
    } else {	      # plink
      #  print STDERR "first allele $first_allele \n";
      push @genotypes_this_row, $dosage;
      $row_gts_str .= "$dosage";
  ...
  my $marker_total_allele_count = $ploidy*((scalar @genotypes_this_row) - $marker_missing_data_count);
  ...
  die "# n col ids: ", scalar @col_ids, "; n dosages in row: ", scalar @genotypes_this_row, "\n" if(scalar @genotypes_this_row != scalar @col_ids);
  #  print "# n col ids: ", scalar @col_ids, "; n dosages in row: ", scalar @genotypes_this_row, "\n";
  #sleep(1);
 # $avg_pref_gt_prob /= $pgtprob_count;
  my $marker_missing_data_fraction = $marker_missing_data_count/(scalar @genotypes_this_row);
  ...
  #  print STDERR "row size, total_size: ", size(\@genotypes_this_row), "  ", total_size(\@genotypes_this_row), "\n";
 #   print STDERR "row str size, total_size: ", size(\$row_gts_str), "  ", total_size(\$row_gts_str), "\n";
   # push @rows, \@genotypes_this_row;
    push @row_strings, $row_gts_str;
 #   print STDERR $row_gts_str, "\n";
 #   print STDERR "rows so far. size, total_size:", size(\@rows), "  ", total_size(\@rows), "\n";
  ...
  #  my $n_elems_out =  scalar @row_strings  *  length $row_gts_str;
...
$info_string .= "# ", scalar @row_strings . " rows stored to be output.\n";
...
my ($n_rows_out, $n_cols_out) = (scalar @row_strings, scalar @col_ids);
...
if (! $transpose) {
  ...
  die if(scalar @row_ids_out  !=  scalar @row_strings);
  while (my ($i, $row_dosages) = each(@row_strings)) {
  ...
  while (my ($i, $col_id) = each @col_ids) {
    ...
    if($plink_format){
      ...
      print $fhout "0\t0\t0\t0\t"; # vcftools always has these 4 zeroes - not sure why or whether necessary
    }else{
      print $fhout "$col_id  ";
    }
	...
	print $fhout "$gtstr\n";
      ...
      # my $s = '';
      # while(my($j, $r) = each @row_strings){
      # 	#print $fhout join('', @$r), "\n";
      # 	$s .= sprintf("%s", substr($r, $i, 1)->[$i]);
      # 	# print $fhout $r->[$i];
      # }
      # $s =~ s/\t\s*$//;
      # print $fhout $s, "\n";
    ...
    while(my($imrkr, $r) = each @row_strings){
      my $d = substr($r, $i, 1);
      print $fhout "$d ";
    }
    print $fhout "\n";
