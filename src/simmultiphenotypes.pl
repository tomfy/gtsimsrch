...
# my $uniq_binary_fileset = undef;
my $n_reps = 1;
#my $n_causal_snps = 2;
#my $multiple_chromosomes = 1;
my $n_phenotypes = 2;
...
GetOptions(
	   ...
	   #'uniq_fileset_bin=s' => \$uniq_binary_fileset,
	   'n_reps=i' => \$n_reps,
	   #'n_causal_snps=i' => \$n_causal_snps,
	   #'multiple_chromosomes!' => \$multiple_chromosomes,
	   'n_phenotypes=i' => \$n_phenotypes,
	   'causal_snps=s' => \$causal_snps_file,
	  );
...
my %accid_outstring = ();
  ...
  if (!defined $causal_snps_file) {
    ...
    $causal_variants_filename = $causal_snps_file . "_x";
  ...
  $phenotype_count = 0;
  $simu_command = "Simu  --bfile $binary_fileset  --qt  --causal-variants $causal_variants_filename  --out $outfile ";
  ...
  while(my ($id, $phs) = each %accid_phenotypes){
    if(!exists $accid_outstring{$id}){
      $accid_outstring{$id} = "$id   $n_phenotypes  ";
    }
    $accid_outstring{$id} .= sum(@$phs)/$n_phenotypes . "  " . join(" ", @$phs) . "  ";
  }
#}

  my @sorted_ids = sort keys %accid_outstring;
  open my $fhout, ">", $n_phenotypes . "_phenotypes_" . $n_reps . "_reps";
