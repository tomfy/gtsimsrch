...
void read_dosages_file_and_add_to_genotypesset(char* input_filename, GenotypesSet* the_genotypes_set){
  ...
  if(the_genotypes_set->marker_missing_data_counts == NULL){    
    ...
    //  }
    // if(the_genotypes_set->marker_alt_allele_counts == NULL){    
    the_genotypes_set->marker_alt_allele_counts = construct_vlong_zeroes(marker_ids->size);
    the_genotypes_set->marker_dose_counts = (Vlong**)malloc((the_genotypes_set->ploidy+1)*sizeof(Vlong*));
    for(long i=0; i<=the_genotypes_set->ploidy; i++){
  ...
  while((nread = getline(&line, &len, g_stream)) != -1){
    ...
    // fprintf(stderr, "#mrkerid count: %ld \n", markerid_count); getchar();
    ...
    while(1){ // read dosages from one line.
      ...
      /* 	accession_missing_data_count++; */
      /* } */
      /* // fprintf(stderr, "dosage: %lf  %lf  %c\n", dosage, delta, genotypes[marker_count]); */
      /* }else{ */
      //	char a = token_to_dosage(token, the_genotypes_set->ploidy);
	
	genotypes[marker_count] = token_to_dosage(token, the_genotypes_set->ploidy);
	//	fprintf(stderr, "marker_count: %ld  a: %d\n", marker_count, genotypes[marker_count]);
	...
	//	genotypes[marker_count] = a;
	//	fprintf(stderr, "#### marker_count: %ld  a: %d\n", marker_count, genotypes[marker_count]);
	//	fprintf(stderr, "#::: %ld [%s] %ld \n", marker_count, genotypes, strlen(genotypes));
	/* for(int i=0; i<40; i++){ */
	/*   fprintf(stderr, "%d", genotypes[i]); */
	/* }fprintf(stderr, "\n"); */
	//  }
      marker_count++;
      // fprintf(stderr, "#### marker_count: %ld  a: %d\n", marker_count, genotypes[marker_count]);
...
char token_to_dosage(char* token, long ploidy){
  if(strcmp(token,"NA") == 0){ // missing data
    ...
    char dosage = atoi(token);
    if(dosage < 0  ||  dosage > ploidy){ // valid dosages are 0, 1, ... ploidy
      return MISSING_DATA_CHAR; // out of range -> missing data
    }
    return dosage + 48; // so dosage 0 becomes char '0', 1 -> '1' etc.
  }
}

double ragmr(GenotypesSet* the_gtsset){
  //  fprintf(stderr, "top of ragmr\n");
...
void clean_genotypesset(GenotypesSet* the_gtsset){ // construct a new set of 'cleaned' accessions, which replace the raw accessions
  ...
  for(long i=0; i<md_counts->size; i++){
    ...
    if(
       (md_counts->a[i] <= max_marker_md_fraction*the_gtsset->n_accessions)
       &&
       ( (alt_allele_counts->a[i] >= min_min_allele_count)  &&
	 (alt_allele_counts->a[i] <= max_min_allele_count) )
       ) {
      md_ok->a[i] = 1;
      n_markers_to_keep++;
