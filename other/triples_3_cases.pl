...
# 2: accession, offspring1, offspring2
# 3: accession, parent, offspring

# typical usage:  triple_3_cases.pl  2 < find_parents.out > aoo_pedigrees
...
while (<>) { # read find_parents output file and add edges to the graph
  ...
  my @cols = split(" ", $_);
  my ($aid, $pid1, $pid2) = @cols[0,2,3]; # @cols[0,2,3];
  next if($aid eq 'NA');
  $ids{$aid} = 1;
  if ($pid1 ne 'NA') {
    $ids{$pid1} = 1;
    $g->add_edge($pid1, $aid); # edge goes from parent to offspring
    $edge_count++;
  }
  if ($pid2 ne 'NA') {
    $ids{$pid2} = 1;
    $g->add_edge($pid2, $aid); # edge goes from parent to offspring
    $edge_count++;
  }
}
